...
class ApiContractTest {
    ...
    @Test
    fun `POST api email should accept exact challenge format`() {
        // Given: Exact challenge format from specification
        val challengeRequest = EmailIngestRequest(
            booking = "BK123",
            containers = listOf(
        ...
        val response = restTemplate.postForEntity(
            "${baseUrl()}/api/email",
            HttpEntity(challengeRequest, authHeaders()),
            Map::class.java
        )
    ...
    fun `POST api email should handle partial data - only booking`() {
        ...
        val partialRequest = EmailIngestRequest(
            booking = "BK456",
            containers = null,
            orders = null
        )
