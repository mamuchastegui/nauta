...
class ProgressiveLinkingTest {
    ...
    fun `should link orders and containers with same booking reference`() {
        ...
        val container1 = containerRepository.findByContainerRef(TENANT_ID, com.nauta.takehome.domain.ContainerRef("LINK1234567"))
        val container2 = containerRepository.findByContainerRef(TENANT_ID, com.nauta.takehome.domain.ContainerRef("LINK7654321"))
    ...
    fun `should use fallback linking when no booking reference provided`() {
        ...
        val order = orderRepository.findByPurchaseRef(TENANT_ID, com.nauta.takehome.domain.PurchaseRef("PO-FALLBACK-001"))
        val container = containerRepository.findByContainerRef(TENANT_ID, com.nauta.takehome.domain.ContainerRef("FALL1234567"))
        assertNotNull(order)
    ...
    fun `should handle partial linking when only some entities have booking reference`() {
        ...
        val message = IngestMessage(
            ...
            orders = listOf(
                OrderData("PO-PARTIAL-001", emptyList()) // Will get booking ref
            ),
            containers = listOf(
                ContainerData("PART1234567") // Will get booking ref
            )
        )
        ...
        val order = orderRepository.findByPurchaseRef(TENANT_ID, com.nauta.takehome.domain.PurchaseRef("PO-PARTIAL-001"))
        val container = containerRepository.findByContainerRef(TENANT_ID, com.nauta.takehome.domain.ContainerRef("PART1234567"))
        assertNotNull(order)
        assertNotNull(container)
    ...
    fun `should handle empty orders or containers gracefully`() {
        ...
        val ordersOnlyMessage = IngestMessage(
            tenantId = TENANT_ID,
        ...
        val order = orderRepository.findByPurchaseRef(TENANT_ID, com.nauta.takehome.domain.PurchaseRef("PO-ORDERS-ONLY"))
        assertNotNull(order)
        ...
        val containersOnlyMessage = IngestMessage(
            tenantId = TENANT_ID,
        ...
        val container = containerRepository.findByContainerRef(TENANT_ID, com.nauta.takehome.domain.ContainerRef("CONT1234567"))
        assertNotNull(container)
    ...
    fun `should handle complex multi-entity scenarios`() {
        ...
        val complexMessage = IngestMessage(
            tenantId = TENANT_ID,
        ...
        val order1 = orderRepository.findByPurchaseRef(TENANT_ID, com.nauta.takehome.domain.PurchaseRef("PO-COMPLEX-001"))!!
        val containersForOrder1 = orderContainerRepository.findContainersByOrderId(TENANT_ID, order1.id!!)
        assertEquals(4, containersForOrder1.size)

        // Verify: Each container is linked to all orders
        val container1 = containerRepository.findByContainerRef(TENANT_ID, com.nauta.takehome.domain.ContainerRef("COMP1234567"))!!
